...
#[derive(Debug)]
struct MockValue {
    pub popcb_called_times : u32,
    pub poppablecb_called_times : u32,
    pub poppable : bool,
    pub key : u64,
}

fn incr(n : &mut u32) { *n = *n + 1; }

impl CacheValue for MockValue {
    type KeyType = u64;
    fn pop_callback(&mut self) { incr(&mut self.popcb_called_times) }
    fn poppable(&mut self) -> bool { incr(&mut self.poppablecb_called_times); self.poppable }
}

impl MockValue {
    fn new(k : u64) -> Self {
        MockValue{
            popcb_called_times : 0,
            poppablecb_called_times : 0,
            poppable : true,
            key : k,
        }
    }
